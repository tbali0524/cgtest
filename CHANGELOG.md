...
- `cgtest-version` configuration option added to the global section.
- `log-single` configuration option added to the global section.
- `--log-single` command-line argument added.

### Changed

- Default config for `php` tweaked, use `--alt` to run without JIT.
...
- Fixed php warning when custom language name is found in the config file.
